...
namespace HKX2
{
    public class PackFileSerializer
    {
        ...
        public void Serialize(IHavokObject rootObject, BinaryWriterEx bw, HKXHeader header)
        {
            ...
            {
                while (_serializationQueues.Count > 1 && _serializationQueues[^1].Count == 0)
                {
                    _serializationQueues.RemoveAt(_serializationQueues.Count - 1);
                }

                var sq = _serializationQueues[^1];
                ...
                {
                    while (_localWriteQueues.Count > 1 && _localWriteQueues[^1].Count == 0)
                    {
        ...
        public void WriteClassPointer<T>(BinaryWriterEx bw, T? d) where T : IHavokObject
        {
            ...
            if (_globalLookup.TryGetValue(d, out var value))
            {
                var gfu = new GlobalFixup { Src = pos, DstSectionIndex = 2, Dst = value };
                _globalFixups.Add(gfu);
                return;
            }
