...
namespace HKX2
{
    public class XmlSerializer
    {
        ...
        private Dictionary<IHavokObject, string> _serializedObjectsLookup = null!;
        private XDocument _document = null!;
        private XContainer _dataSection = null!;
        ...
        public void Serialize(IHavokObject rootObject, HKXHeader header, Stream stream)
        {
            ...
            var hkPackFile = _document.Element("hkpackfile");
            if (hkPackFile is null) {
                throw new InvalidOperationException("Missing required element 'hkpackfile' in XDocument.");
            }
            var hkSection = hkPackFile.Element("hksection");
            if (hkSection is null) {
                throw new InvalidOperationException("Missing required element 'hksection' under 'hkpackfile' in XDocument.");
            }
            _dataSection = hkSection;
        ...
        public void WriteClassPointer<T>(XElement xe, string paramName, T? value) where T : IHavokObject
        {
            ...
            if (_serializedObjectsLookup.TryGetValue(value, out var index1))
            {
                var hkparam = WriteString(xe, paramName, index1);
            }
            else
            {
                var index = GetIndex();
        ...
        public void WriteClassPointerArrayNullable<T>(XElement xe, string paramName, IList<T?> values) where T : IHavokObject
        {
            ...
            foreach (var item in values)
            {
                ...
                if (_serializedObjectsLookup.TryGetValue(item, out var value))
                {
                    indexs.Add(value);
                    continue;
                }
