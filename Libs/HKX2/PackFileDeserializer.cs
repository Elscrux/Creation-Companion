...
namespace HKX2
{
    public class PackFileDeserializer
    {
        ...
        private HKXSection _dataSection = null!;

        private Dictionary<uint, IHavokObject> _deserializedObjects = null!;
        public HKXHeader _header = null!;
        private HKXSection _typeSection = null!;

        private bool _ignoreNonFatalError;

        private IHavokObject ConstructVirtualClass(BinaryReaderEx br, uint offset)
        {
            if (_deserializedObjects.TryGetValue(offset, out var o)) return o;
            ...
            if (hkClass is null) throw new InvalidOperationException($"Havok class type '{hkClassName}' not found!");

            var ret = Activator.CreateInstance(hkClass) as IHavokObject;
            if (ret is null) throw new InvalidOperationException($"Failed to Activator.CreateInstance({hkClass})");
        ...
        private T ReadPointerBase<T, F>(Func<BinaryReaderEx, F, T> func, BinaryReaderEx br) where F : Fixup, new()
        {
            ...
            if (!map.TryGetValue(key, out var f))
            {
        ...
        public T ReadClassPointer<T>(BinaryReaderEx br) where T : IHavokObject
        {
            ...
            if (!_dataSection._globalMap.TryGetValue(key, out var f)) return default!;
            ...
            if (!_ignoreNonFatalError)
                throw new InvalidOperationException($"Could not convert '{typeof(T)}' to '{klass.GetType()}'. Is source malformed?");
        ...
        public string ReadStringPointer(BinaryReaderEx br)
        {
            ...
            if (!_dataSection._localMap.TryGetValue(key, out var f)) return string.Empty;
        ...
        public string ReadCString(BinaryReaderEx br)
        {
            ...
            // Do a local fixup lookup
            if (!_dataSection._localMap.TryGetValue(key, out var f)) return null!;
