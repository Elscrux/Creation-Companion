...
public sealed class RaceFilter : SimpleRecordFilter<IRaceGetter> {
    public RaceFilter() : base(new List<SimpleRecordFilterEntry> {
        ...
        new("NPC",
            record =>
                record.Keywords is not null
             && record.Keywords.Contains(Mutagen.Bethesda.FormKeys.SkyrimSE.Skyrim.Keyword.ActorTypeNPC)),
        new("Creature",
            record =>
                record.Keywords is not null
             && record.Keywords.Contains(Mutagen.Bethesda.FormKeys.SkyrimSE.Skyrim.Keyword.ActorTypeCreature)),
    }) {}
}
